...
Почему красиво?
- Добавляется небольшой кусок разметки, подтягивающий нужные секреты
- Получаем крутую секретохранилку

Что имеем без секретохранилки?
- В GitHub Secrets все секреты репозитория доступны для всех джобов и шагов. Нет лёгкого способа изолировать секреты между этапами пайплайна.
- GitHub не предоставляет подробного логирования использования секретов. Мы не знаем, кто и когда использовал секрет.
- Секреты, сохраненные в GitHub Secrets, статичны. Если ключ или токен скомпрометирован, его необходимо вручную заменить во всех репозиториях.
- Хранение секретов в GitHub Secrets увеличивает их количество, что усложняет управление. Например, для каждого репозитория вам нужно хранить уникальные ключи для баз данных, API и т.д.
- Секреты привязаны к одному репозиторию или организации.

Что имеем с ней?
- Вместо хранения всех секретов в GitHub Secrets, вы храните только один кред доступа к Vault, который предоставляет доступ к множеству других секретов. Это позволяет централизовать управление секретами.
- Vault может автоматически генерировать временные (динамические) секреты с ограниченным сроком действия.
- Vault позволяет задавать тонко настроенные политики доступа к секретам.
- Vault умеет в аудит (кто, что, когда)
- Vault позволяет изолировать секреты для разных окружений, этапов или приложений.
- Vault можно использовать как единое хранилище секретов для всех систем, облачных провайдеров и платформ.

В итоге зачем?
Получаем крутое централизованное универсальное управление секретами. Хотя для небольших приложений смысл поднимать инстанс волта я не вижу. (экспириенс был прикольный)
